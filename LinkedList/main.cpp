...
int main() {
  ...
  string line;
  char code[32], name[32], type[32], temp[32], temp2[32], temp3[32], temp4[32],
      temp5[32], temp6[32], temp7[32];
  double latitude, longitude, tempInt;

  infile.open("AdvCS203-2024/LASAlle/airport-codes_US.csv", ifstream::in);
  if (infile.is_open()) {
    int c = 0;
    // getline( infile, line );
    while (getline(infile, line)) {
      if (c < 22337) {
        ...
        if (sscanf(line.c_str(), "%[^,],%[^,],%[^,],%lf,%lf", code, type, name,
                   &latitude, &longitude) == 5) {
        }

        // cout << code << " "<< type << " "<< name << " " << latitude << " " <<
        // longitude << endl;

        Airport *newAirport =
            new Airport(code, name, type, latitude, longitude);
    ...
    // cout << "Furthest Airport: " << furthestAirport->name << " " <<
    // furthestAirport -> distance << endl;
